    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if products and len(products) > 0:
                ...
                logger.info(
                    f"Product: {name}, Price: {price}, Min: {min_quantity}, "
                    f"Stock: {stock_status}, Qty: {stock_quantity}"
                )
                return {
                    'product_id': sku,
                    'name': name,
...
def create_order(user_id, cart_items):
    ...
    try:
        total = sum(
            ...
            for item in cart_items if item['price']
        )
        with conn.cursor() as cursor:
            cursor.execute(
                "INSERT INTO orders "
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                "o.created_at, COUNT(oi.item_id) as item_count "
                "FROM orders o "
...
def format_cart(cart_items):
    ...
    for idx, item in enumerate(cart_items, 1):
        ...
        text += (
            f"**{idx}- {product_name}**\n\n"
            f"**{quantity} x {price:,.0f} = {subtotal:,.0f}**\n"
            f"➖➖➖➖➖➖➖\n"
        )
    ...
    if context.args and context.args[0].startswith('product_'):
        ...
        if product_info:
            ...
            if not product_info.get('in_stock', False):
                ...
                stock_msg = ""
                if (product_info.get('manage_stock') and
                        product_info.get('stock_quantity')):
...
async def handle_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('awaiting_new_quantity'):
        ...
        if new_quantity < min_qty:
            await update.message.reply_text(
                f"**❌ حداقل {min_qty} عدد!**\n\n"
                f"لطفاً تعداد بیشتری وارد کنید:",
                parse_mode='Markdown'
            )
            return
    ...
    if add_to_cart(user_id, product_info['product_id'], quantity):
        ...
        keyboard = [
            [
                InlineKeyboardButton("➕ افزودن", callback_data="add_more"),
                InlineKeyboardButton("✏️ ویرایش", callback_data="edit_cart")
            ],
            [
                InlineKeyboardButton(
                    "✅ تکمیل",
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "add_more":
        ...
        keyboard.append([
            InlineKeyboardButton("🔙 بازگشت", callback_data="back_to_cart")
        ])
        await query.edit_message_text(
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if contact:
        ...
        if order_id:
            ...
            user_info = get_user_info(user_id)
            user_name = (
                user_info.get('first_name', 'مشتری')
                if user_info else 'مشتری'
            )
            confirmation = (
                f"**✅ سفارش ثبت شد!**\n\n"
                f"**📋 شماره: {order_id:04d}**\n"
...
async def show_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for order in orders:
        ...
        text += (
            ...
            f"{order['total_amount']:,.0f} تومان - "
            f"{order['item_count']} محصول\n"
            f"➖➖➖➖➖\n\n"
        )
...
async def register_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user_info = get_user_info(user_id)
    if (user_info and user_info.get('phone_number') and
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (context.user_data.get('awaiting_quantity') and
            text in ["🛒 سفارشات من", "📝 ثبت نام", "📞 پشتیبانی"]):
        ...
        if text == "🛒 سفارشات من":
            ...
            admin_username = (
                ADMIN_USERNAME if ADMIN_USERNAME.startswith('@')
                else f'@{ADMIN_USERNAME}'
            )
            await update.message.reply_text(
    ...
    if context.user_data.get('awaiting_quantity'):
        await handle_quantity(update, context)
        return
    if (context.user_data.get('awaiting_phone') or
            context.user_data.get('registering') or
            context.user_data.get('editing_phone')):
        phone = text.strip()
    ...
    if text == "🛒 سفارشات من":
        ...
        await update.message.reply_text(
            "**لطفاً از منو یا کانال انتخاب کنید.**",
            parse_mode='Markdown'
        )
